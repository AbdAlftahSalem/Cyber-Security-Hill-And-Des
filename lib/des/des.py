...
class des(_baseDes):
    ...
    def subKeyMethod(self):
        key = self.permutateMethod(des.__pc1, self.stringToList(self.getKey()))
        i = 0
        self.L = key[:28]
        self.R = key[28:]
        while i < 16:
            ...
            self.Kn[i] = self.permutateMethod(des.__pc2, self.L + self.R)

            i += 1

    def __des_crypt(self, block, crypt_type):
        block = self.permutateMethod(des.__ip, block)
        self.L = block[:32]
        self.R = block[32:]
        ...
        while i < 16:
            ...
            self.R = self.permutateMethod(des.__expansion_table, self.R)
            ...
            self.R = self.permutateMethod(des.__p, Bn)
        ...
        self.final = self.permutateMethod(des.__fp, self.R + self.L)
        return self.final
    ...
    def crypt(self, data, crypt_type):
        ...
        if self.getMode() == CBC:
            if self.getIV():
                iv = self.stringToList(self.getIV())
            else:
        ...
        while i < len(data):
            ...
            block = self.stringToList(data[i:i + 8])
            ...
            result.append(self.bitToList(processed_block))
